...
int Eval::evaluate(Board &b)
{
    ...
    int whiteSpaceScore = (SPACE_BONUS[0][1] * count(whiteSpace & CENTER_FILES) + SPACE_BONUS[0][0] * count(whiteSpace & ~CENTER_FILES) + SPACE_BONUS[1][1] * count(whiteSpace2 & CENTER_FILES) + SPACE_BONUS[1][0] * count(whiteSpace2 & ~CENTER_FILES)) * whiteSpaceWeight * whiteSpaceWeight / 512;
    ...
    int blackSpaceScore = (SPACE_BONUS[0][1] * count(blackSpace & CENTER_FILES) + SPACE_BONUS[0][0] * count(blackSpace & ~CENTER_FILES) + SPACE_BONUS[1][1] * count(blackSpace2 & CENTER_FILES) + SPACE_BONUS[1][0] * count(blackSpace2 & ~CENTER_FILES)) * blackSpaceWeight * blackSpaceWeight / 512;
        ...
        for (int color = WHITE; color <= BLACK; color++)
        {
            ...
            for (int i = fileRange - 1; i <= fileRange + 1; i++)
            {
                ...
                if (pawnStorm)
                {
                    ...
                    ksValue[color] -= PAWN_STORM_VALUE[(pieces[color][PAWNS] & FILES[i]) == 0 ? 0 : (pieces[color][PAWNS] & indexToBit(stopSq)) != 0 ? 1
                                                                                                                                                     : 2][f][r];

                    if (f == 0 && (kingFile == 0 || kingFile == 7) && (r == 1 || r == 2) && relativeRank(color, kingRank) + 1 == r)
    ...
    pieceEvalScore[WHITE] += SHIELDED_MINOR_BONUS * count((pieces[WHITE][PAWNS] >> 8) & (pieces[WHITE][KNIGHTS] | pieces[WHITE][BISHOPS]) & (RANK_2 | RANK_3 | RANK_4));
    pieceEvalScore[BLACK] += SHIELDED_MINOR_BONUS * count((pieces[BLACK][PAWNS] << 8) & (pieces[BLACK][KNIGHTS] | pieces[BLACK][BISHOPS]) & (RANK_7 | RANK_6 | RANK_5));

    // Outposts
    constexpr uint64_t OUTPOST_SQS[2] = {(CENTER_FILES & (RANK_4 | RANK_5 | RANK_6)) | ((FILE_B | FILE_G) & (RANK_5 | RANK_6)),
    ...
    for (int color = WHITE; color <= BLACK; color++)
    {
        ...
        uint64_t mobilitySafeSqs = ~(pieceRammedPawns[color] | pieces[color][KINGS] | ei.attackMaps[color ^ 1][PAWNS] | (ei.doubleAttackMaps[color ^ 1] & ~ei.doubleAttackMaps[color]));
        ...
        for (unsigned int i = 0; i < pml.starts[BISHOPS]; i++)
        {
            ...
            mobilityScore[color] += MOBILITY[KNIGHTS - 1][count(mobilityMap)] + EXTENDED_CENTER_VAL * count(mobilityMap & EXTENDED_CENTER_SQS) + CENTER_BONUS * count(mobilityMap & CENTER_SQS);
        ...
        for (unsigned int i = pml.starts[BISHOPS]; i < pml.starts[ROOKS]; i++)
        {
            ...
            mobilityScore[color] += MOBILITY[BISHOPS - 1][count(mobilityMap)] + EXTENDED_CENTER_VAL * count(mobilityMap & EXTENDED_CENTER_SQS) + CENTER_BONUS * count(mobilityMap & CENTER_SQS);
        ...
        for (unsigned int i = pml.starts[ROOKS]; i < pml.starts[QUEENS]; i++)
        {
            ...
            mobilityScore[color] += MOBILITY[ROOKS - 1][count(mobilityMap)] + EXTENDED_CENTER_VAL * count(mobilityMap & EXTENDED_CENTER_SQS) + CENTER_BONUS * count(mobilityMap & CENTER_SQS);
        ...
        for (unsigned int i = pml.starts[QUEENS]; i < pml.size(); i++)
        {
            ...
            if (ei.attackMaps[color ^ 1][KNIGHTS] & b.getKnightSquares(queenSq) & ~ei.attackMaps[color][PAWNS] & ~(ei.doubleAttackMaps[color] & ~ei.doubleAttackMaps[color ^ 1]))
    ...
    for (int color = WHITE; color <= BLACK; color++)
    {
        ...
        if (uint64_t threatened = (pieces[color][KNIGHTS] | pieces[color][BISHOPS] | pieces[color][ROOKS] | pieces[color][QUEENS]) & ei.attackMaps[color ^ 1][PAWNS])
    ...
    // Pawn phalanxes
    uint64_t wPawnPhalanx = (pieces[WHITE][PAWNS] & (pieces[WHITE][PAWNS] << 1) & NOTA) | (pieces[WHITE][PAWNS] & (pieces[WHITE][PAWNS] >> 1) & NOTH);
    uint64_t bPawnPhalanx = (pieces[BLACK][PAWNS] & (pieces[BLACK][PAWNS] << 1) & NOTA) | (pieces[BLACK][PAWNS] & (pieces[BLACK][PAWNS] >> 1) & NOTH);
    while (wPawnPhalanx)
    {
    ...
    if (egFactor > 0)
    {
        ...
        int oppositionDistance = std::abs((kingSq[WHITE] & 7) - (kingSq[BLACK] & 7)) - std::abs((kingSq[WHITE] >> 3) - (kingSq[BLACK] >> 3));

        int egWinAdjustment = PAWN_ASYMMETRY_BONUS * pawnAsymmetry + PAWN_COUNT_BONUS * (pieceCounts[WHITE][PAWNS] + pieceCounts[BLACK][PAWNS]) + KING_OPPOSITION_DISTANCE_BONUS * oppositionDistance + ENDGAME_BASE;
    ...
    {
        if (pieceCounts[WHITE][BISHOPS] == 1 && pieceCounts[BLACK][BISHOPS] == 1 && (((pieces[WHITE][BISHOPS] & LIGHT) && (pieces[BLACK][BISHOPS] & DARK)) || ((pieces[WHITE][BISHOPS] & DARK) && (pieces[BLACK][BISHOPS] & LIGHT))))
        {
            if ((b.getNonPawnMaterial(WHITE) == pieces[WHITE][BISHOPS]) && (b.getNonPawnMaterial(BLACK) == pieces[BLACK][BISHOPS]))
    ...
    for (int color = WHITE; color <= BLACK; color++)
    {
        if (material[MG][color] - material[MG][color ^ 1] > 0 && material[MG][color] - material[MG][color ^ 1] <= PIECE_VALUES[MG][KNIGHTS] && pieceCounts[color][PAWNS] <= 1 && totalEval * (1 - 2 * color) > 0)
        {
...
int Eval::getKingSafety(Board &b, PieceMoveList &attackers, uint64_t kingSqs, int pawnScore, int kingFile)
{
    ...
    uint64_t weakMap = ~ei.doubleAttackMaps[attackingColor ^ 1] & ((~ei.attackMaps[attackingColor ^ 1][PAWNS] & ~ei.fullAttackMaps[attackingColor ^ 1]) | ei.attackMaps[attackingColor ^ 1][QUEENS]);
    ...
    kingSafetyPts += SAFE_CHECK_BONUS[QUEENS - 1] * !!(ei.attackMaps[attackingColor][QUEENS] & checkMaps[QUEENS - 1] & ~kingSqs & weakMap & ~ei.attackMaps[attackingColor ^ 1][QUEENS]);
    ...
    int kingPressure = KING_PRESSURE * (count(ei.fullAttackMaps[attackingColor] & kingZone) + count(ei.doubleAttackMaps[attackingColor] & ~ei.attackMaps[attackingColor ^ 1][PAWNS] & kingZone));
    ...
    kingSafetyPts += KS_NO_KNIGHT_DEFENDER * (!(kingDefenseZone & ei.attackMaps[attackingColor ^ 1][KNIGHTS]) + !(kingZone & ei.attackMaps[attackingColor ^ 1][KNIGHTS])) * pieceCounts[attackingColor ^ 1][KNIGHTS];
    kingSafetyPts += KS_NO_BISHOP_DEFENDER * (!(kingDefenseZone & ei.attackMaps[attackingColor ^ 1][BISHOPS]) + !(kingZone & ei.attackMaps[attackingColor ^ 1][BISHOPS])) * pieceCounts[attackingColor ^ 1][BISHOPS];
...
int Eval::checkEndgameCases()
{
    ...
    else if (numPieces == 2)
    {
        ...
        if (numWPieces == 1)
        {
            ...
            if ((pieces[WHITE][KNIGHTS] | pieces[WHITE][BISHOPS]) && (pieces[BLACK][KNIGHTS] | pieces[BLACK][BISHOPS]))
                return 0;
        ...
        else
        {
            ...
            if (pieces[WHITE][PAWNS])
            {
                ...
                int wf = wPawnSq & 7;
                int wr = wPawnSq >> 3;
                if (pieces[WHITE][BISHOPS] && ((wf == 0 && (pieces[WHITE][BISHOPS] & DARK)) || (wf == 7 && (pieces[WHITE][BISHOPS] & LIGHT))))
                {
            ...
            if (pieces[BLACK][PAWNS])
            {
                ...
                int bf = bPawnSq & 7;
                int br = bPawnSq >> 3;
                if (pieces[BLACK][BISHOPS] && ((bf == 0 && (pieces[BLACK][BISHOPS] & LIGHT)) || (bf == 7 && (pieces[BLACK][BISHOPS] & DARK))))
                {
