...
int probe_dtz(const Board &b, int *success)
{
    ...
    if (wdl > 0)
    {
        ...
        for (unsigned int i = 0; i < legalMoves.size(); i++)
        {
            Move move = legalMoves.get(i);
            Board copy = b.staticCopy();
            if (b.getPieceOnSquare(color, getStartSq(move)) != PAWNS || isCapture(move) || !copy.doPseudoLegalMove(move, color))
                continue;
    ...
    for (unsigned int i = 0; i < legalMoves.size(); i++)
    {
        ...
        if (isCapture(move) || b.getPieceOnSquare(color, getStartSq(move)) == PAWNS || !copy.doPseudoLegalMove(move, color))
            continue;
