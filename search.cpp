                 ...
                 int tbScore, bool tbProbeSuccess, int threadID)
{
    ...
    do
    {
        ...
        for (unsigned int multiPVNum = 1;
             multiPVNum <= multiPV && multiPVNum <= legalMoves.size();
             multiPVNum++)
        {
            ...
            while (!isStop)
            {
                ...
                else if (bestScore >= aspBeta)
                {
                    ...
                    if (threadID == 0 && (int)multiPVNum - 1 == bestMoveIndex && bestMove == prevBest && timeParams->searchMode == TIME && (timeSoFar >= timeParams->allotment * TIME_FACTOR))
                        break;
            ...
            for (unsigned int i = 0; i < legalMoves.size(); i++)
            {
                ...
                if (isCapture(m))
                {
                    ...
                    scores.add(16384 + adjustedMVVLVA + threadMemoryArray[threadID]->searchParams.captureHistory[color][pieceID][capturedID][endSq]);
                }
        ...
        if (threadID == 0 && !isPonderSearch && timeParams->searchMode == TIME && multiPV == 1 && pvStreak >= 8 + rootDepth / 5 && timeSoFar > (uint64_t)timeParams->allotment / 16 && timeSoFar < (uint64_t)timeParams->allotment / 2 && abs(bestScore) < NEAR_MATE_SCORE)
        {
            int secondBestMove;
            int secondBestScore;
    ...
    while (!isStop && ((threadID != 0 && rootDepth <= MAX_DEPTH) || ((((timeParams->searchMode == TIME && timeSoFar < (uint64_t)timeParams->allotment * TIME_FACTOR * timeChangeFactor) || isPonderSearch) && rootDepth <= MAX_DEPTH) || (timeParams->searchMode == MOVETIME && timeSoFar < (uint64_t)timeParams->allotment && rootDepth <= MAX_DEPTH) || (timeParams->searchMode == DEPTH && rootDepth <= timeParams->allotment))));
                        ...
                        int threadID, SearchPV *pvLine)
{
    ...
    *bestScore = -INFTY;
    ...
    for (unsigned int i = startMove; i < legalMoves->size(); i++)
    {
        ...
        int reduction = 0;
        if (depth >= 3 && movesSearched > 1 && !isCapture(m) && !isPromotion(m))
        {
        ...
        // Check extensions
        int extension = 0;
        if (copy.isInCheck(color ^ 1) && b->isSEEAbove(color, m, 0))
        {
            extension++;
        }
...
int PVS(Board &b, int depth, int alpha, int beta, int threadID, bool isCutNode, SearchStackInfo *ssi, SearchPV *pvLine)
{
    ...
    if (probeLimit && count(b.getAllPieces(WHITE) | b.getAllPieces(BLACK)) <= probeLimit && b.getFiftyMoveCounter() == 0 && !b.getAnyCanCastle())
    {
        int tbProbeResult;
    ...
    bool evalImproving = ssi->ply >= 3 && !isInCheck && (ssi->staticEval >= (ssi - 2)->staticEval || (ssi - 2)->staticEval == INFTY);
    ...
    if (!isPVNode && !isInCheck && depth <= 6 && staticEval - 70 * depth >= beta && b.getNonPawnMaterial(color))
        return staticEval;
    ...
    // as much.
    if (!isPVNode && !isInCheck && depth <= 2 && staticEval <= alpha - RAZOR_MARGIN)
    {
        searchParams->ply = ssi->ply;
        if (depth == 1)
            return quiescence(b, 0, alpha, beta, threadID);
    ...
    if (!isPVNode && !isInCheck && depth >= 2 && staticEval >= beta && b.getNonPawnMaterial(color))
    {
    ...
    // Idea from Stockfish
    if (!isPVNode && !isInCheck && depth >= 6 && staticEval >= beta - 100 - 20 * depth && abs(beta) < MAX_PLY_MATE_SCORE)
    {
    ...
    if (hashed == NULL_MOVE && !isInCheck && ((isPVNode && depth >= 6) || (!isPVNode && depth >= 8)))
    {
         ...
         m = moveSorter.nextMove())
    {
        ...
        bool moveIsPrunable = !isCapture(m) && !isPromotion(m) && m != hashed && bestScore > -MAX_PLY_MATE_SCORE && !isCheckMove;
        ...
        int pruneDepth = isPVNode ? lmrDepth + 1 : evalImproving ? lmrDepth
                                                                 : lmrDepth - 1;
        ...
        // q-searching it.
        if (moveIsPrunable && !isInCheck && pruneDepth <= 6 && staticEval <= alpha - 115 - 90 * pruneDepth)
            continue;
        ...
        bool doMoveCountPruning = depth <= 12 && movesSearched > LMP_MOVE_COUNTS[evalImproving][depth] + (isPVNode ? depth : 0);
        if (moveIsPrunable && doMoveCountPruning)
            continue;

        // Prune moves with low history
        if (moveIsPrunable && pruneDepth <= 2 && ((ssi->counterMoveHistory != nullptr) ? ssi->counterMoveHistory[pieceID][endSq] : -1) < 0 && ((ssi->followupMoveHistory != nullptr) ? ssi->followupMoveHistory[pieceID][endSq] : -1) < 0)
            continue;

        // Futility pruning using SEE
        if (moveIsPrunable && pruneDepth <= 6 && !b.isSEEAbove(color, m, -24 * pruneDepth * pruneDepth))
            continue;

        if (!isPVNode && m != hashed && bestScore > -MAX_PLY_MATE_SCORE && depth <= 5 && !b.isSEEAbove(color, m, -100 * depth))
            continue;
        ...
        if (depth >= 3 && movesSearched > 1 && !isCapture(m) && !isPromotion(m))
        {
            ...
            if (m == searchParams->killers[ssi->ply][0] || m == searchParams->killers[ssi->ply][1])
                reduction--;
            // Reduce more for moves with poor history
            int historyValue = searchParams->historyTable[color][pieceID][endSq] + ((ssi->counterMoveHistory != nullptr) ? ssi->counterMoveHistory[pieceID][endSq] : 0) + ((ssi->followupMoveHistory != nullptr) ? ssi->followupMoveHistory[pieceID][endSq] : 0);
        ...
        int extension = 0;
        // Check extensions
        if (!doMoveCountPruning && isCheckMove && b.isSEEAbove(color, m, 0))
        {
            extension++;
        }
        ...
        if (depth >= 8 && reduction == 0 && extension == 0 && m == hashed && abs(hashScore) < NEAR_MATE_SCORE && (nodeType == CUT_NODE || nodeType == PV_NODE) && hashDepth >= depth - 3)
        {
...
int quiescence(Board &b, int plies, int alpha, int beta, int threadID)
{
         ...
         m = moveSorter.nextMove())
    {
        ...
        // Futility pruning
        if (!isCheckMove && !isPromotion(m) && staticEval < alpha - 80 && !b.isSEEAbove(color, m, 1))
        {
...
int checkQuiescence(Board &b, int plies, int alpha, int beta, int threadID)
{
    ...
    for (unsigned int i = 0; i < legalMoves.size(); i++)
    {
        ...
        if (bestScore > -MAX_PLY_MATE_SCORE && !isCapture(m) && !b.isSEEAbove(color, m, 0))
            continue;
